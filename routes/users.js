...
function insertUser(user, callback) {
  User.register(new User(user), user.password, function(err, account) {
    if (err) {
      var message = { title: 'notification.newuser.alreadyexists.title', message: 'notification.newuser.alreadyexists.message' }
      callback(message)
    }
...
function updateUser(req, user, callback) {
  ...
  db.collection('users').findAndModify(
    ...
    function(err, object) {
      if (err) {
        var message = { title: 'notification.updateuser.error.title', message: 'notification.updateuser.error.message' }
        callback(message)
      } else {
